...
class KerasSELECT(KerasPilot):
    def __init__(self, model=None, num_outputs=2, num_sel_inputs=1, input_shape=(120, 160, 3), *args, **kwargs):
        super(KerasSELECT, self).__init__(*args, **kwargs)
        self.num_sel_inputs = num_sel_inputs
        self.model = default_sel(num_outputs = num_outputs, num_sel_inputs = num_sel_inputs, input_shape=input_shape)
        self.compile()
    def compile(self):
        self.model.compile(optimizer=self.optimizer,
                  loss='mse')
    def run(self, img_arr, sel):
        #TODO: would be nice to take a vector input array.
        if img_arr is not None:
            img_arr = img_arr.reshape((1,) + img_arr.shape)
            sel_arr = np.array([sel]).reshape(1,self.num_sel_inputs)
            outputs = self.model.predict([img_arr, sel_arr])
            steering = outputs[0]
            throttle = outputs[1]
            return steering[0][0], throttle[0][0]
...
def default_sel(num_outputs, num_sel_inputs, input_shape):
    img_in = Input(shape=input_shape, name='img_in')
    sel_in = Input(shape=(num_sel_inputs,), name="sel_in")
    ...
    model = Model(inputs=[img_in, sel_in], outputs=outputs)
